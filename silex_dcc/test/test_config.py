...
def test_resolve_config(dummy_config):
    ...
    """
    resolved_config = dummy_config.resolve_config("config", "foo", "task_a")
    ...
    assert resolved_config["action"][0][
...
def test_resolve_none_existing_action(dummy_config):
    ...
    """
    resolved_config = dummy_config.resolve_config("config", "foobar", "task_a")
    # Pretty print the resolved config
    pp = pprint.PrettyPrinter(indent=4)
    pp.pprint(resolved_config)
...
def test_resolve_none_existing_task(dummy_config):
    ...
    """
    resolved_config = dummy_config.resolve_config("config", "foo", "task_z")
    # Pretty print the resolved config
    pp = pprint.PrettyPrinter(indent=4)
    pp.pprint(resolved_config)
    ...
    assert len(resolved_config["action"]) == 2
    assert len(resolved_config["pre"]) == 1
    assert resolved_config["action"][0]["string"] == "printing A from foo"
    assert resolved_config["post"][0]["command"] == "test.print"
